...
export default function DelayPredictor() {
  ...
  const handlePredict = async () => {
    setIsLoading(true);
    try {
      // Extract train number from trainId (e.g., "12627" or "RS-402 Intercity Express" → "12627")
      ...
      const response = await apiCall<any>(`/delay/${trainNo}`, {
        ...
        body: JSON.stringify({
          ...
          congestion_level: fogDensity > 70 ? 0.8 : 0.5
        })
      }, {
        predicted_delay_min: Math.floor(Math.random() * 30) + 15 + (fogDensity > 70 ? 20 : 0) + (!signalHealth ? 40 : 0),
        confidence_pct: Math.floor(Math.random() * 20) + 75
      });
      
      setDelayMin(response.predicted_delay_min ?? response.predicted_delay_minutes ?? 47);
      setConfidence(response.confidence_pct ?? response.confidence_score ?? 82);
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoading(false);
